...
class ArticleCategoryAdminView extends View {
	...
	public function adminCreate() {
		$url = '';
		$html = '<h1>Create '.ucwords(Text::translate($this->class)).'</h1><form class="form-create" action="" method="post">';
		...
		$html .= Form::select(array(
			'name'=>'status',
			'title'=>'Status',
			'default'=>(Application::getController()->canPublish() ? STATUS_PUBLISHED : STATUS_UNPUBLISHED),
		...
		$html .= '</div></div><div class="tab-pane" id="publishing-pane" role="tabpanel" aria-labelledby="publishing-tab"><div class="grid-columns"><div>';
		$html .= Form::select(array(
			'name'=>'author',
			'title'=>'Author',
			'prefix'=>'-',
	...
	public function adminEdit() {
		$url = '';
		$html = '<h1>Edit '.ucwords(Text::translate($this->class)).'</h1><form class="form-edit" action="" method="post">';
		$html .= '<div class="form-group"><button class="btn btn-success" id="submit" type="submit" disabled><i class="fa fa-check"></i> Save</button><a href="/admin/'.strtolower($this->class).'" class="btn btn-danger" id="cancel"><i class="fa fa-times"></i> Cancel</a></div>';
		...
		$html .= Form::select(array(
			...
			'class'=>' form-control-sm',
			'query'=>Form::query('articlecategory',Session::isAccessible()),
			'readonly'=>Application::getController()->cannotEdit($this->_data->author)));
		$html .= Form::select(array(
			'name'=>'language',
			'title'=>'Language',
			'prefix'=>'-',
	...
	public function adminList() {
		$html = '<h1>'.ucwords(Text::plural($this->class)).'</h1>';
		...
		foreach($this->_data as $item) {
			...
			$html .= '<a href="/admin/'.$this->class.'?method=edit&id='.$item->id.'" class="btn btn-sm btn-warning'.(Application::getController()->canEdit($item->author) ? '' : ' disabled').'" tabindex="-1"><i class="fa fa-pencil fa-fw"></i></a>';
			$html .= '<a href="/admin/'.$this->class.'?method=trash&id='.$item->id.'" class="btn btn-sm btn-danger'.(Application::getController()->canPublish() ? '' : ' disabled').'" tabindex="-1"><i class="fa fa-trash fa-fw"></i></a>';
			$html .= '</div></div>';
		}
