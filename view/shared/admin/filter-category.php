...
?><div class="form-group">
	...
	<select id="<?php echo $filter['prefix'].$filter['id']; ?>" name="-<?php echo $filter['id']; ?>" class="form-control">
		<option value="<?php echo CATEGORY_ANY; ?>"<?php echo ($filter['value'] == CATEGORY_ANY ? " selected" : ""); ?>>Any category</option>
<?php
$query = "SELECT `id`,`title` FROM `articlecategory` WHERE 1 ORDER BY `title`";
